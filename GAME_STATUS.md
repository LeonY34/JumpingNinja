...
- 当前构建目标：Android
- 状态：核心玩法与菜单流程已实现；地图尺寸、忍者主题背景及十层一换的动态配色已接入，最新 Unity 验证通过
...
- 地图默认宽 15 格，左右边缘由 1×1 白色墙块组成；碰墙后横向速度归零，纵向速度保留。
- 黑色方块会立即结束本局；第 0 层下方为完整致死地板。黑块及致死地板使用克制的深色忍者纹理，并随层数切换色调。所有方块使用与视觉缩放分离的显式尺寸实体碰撞体；Ninja 除碰撞回调外还会在每个物理帧做 Collider 形状扫掠，防止高速跳跃穿透黑块或白墙。
...
- 背景/黑块 Sprite、背景纹样透明度、主题切换层数，以及背景和黑块的主题色组
- 预生成层数
...
- 白色边墙仍保持纯色以确保轮廓清晰；背景与致死黑块已有忍者主题纹理和动态配色，但尚无音效或粒子效果。
