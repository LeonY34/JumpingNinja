...
- 当前构建目标：Android
- 状态：核心玩法与菜单流程已实现；安全白墙与致死黑块均已使用成套忍者纹理，TAP 提示不再阻挡点击，最新 Unity 验证通过
...
- 地图默认宽 15 格，左右边缘由 1×1 浅色忍者石墙组成；墙砖与黑色机关砖采用匹配的边框、斜纹和手里剑图案，并以象牙白/银灰和深黑色保持清晰区分。碰墙后横向速度归零，纵向速度保留。
